...
class onLogin{
	...
	public function onLogin($event){
		...
		if(isset($this->main->def[$name])){
			unset($this->main->def[$name]);
		}
		$this->main->atk[$name] = 1;
		$this->main->def[$name] = 0;
		$this->main->jump[$name] = 0;
		$this->main->lastattack[$name] = 0;
		$folder = $this->main->getFolder($name);
		$this->main->config[$name] = new Config($folder, Config::JSON, array(
			"name" => $name,
			"money" => 500,
			"orb" => 5,
			"Tag" => "",
		...
		foreach($this->main->config as $data){
			$data->save();
		}
		if($this->main->getJob($name) !== ""){
			if(isset($this->main->otu)){
				if($this->main->otu->otu->exists($name)){
					$v = new Vector3(29.5, 20, 97.5);//座標指定
