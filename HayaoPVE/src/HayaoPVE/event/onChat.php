...
class onChat{
	...
	public function onChat($event){
		...
		if(stripos($message, '*/') !== false){
			$player->sendMessage('§chackはおやめください');
			$player->isBanned();
			$event->setCancelled();
		}elseif(stripos($message, '*/bb') !== false){
			$player->sendMessage('§chackはおやめください');
			$player->isBanned();
			$event->setCancelled();
		}elseif(stripos($message, '*/fly') !== false){
			$player->sendMessage('§chackはおやめください');
			$player->isBanned();
			$event->setCancelled();
		}elseif(stripos($message, "/") === false){
			if($this->main->config[$name]->get("guild") !== ""){
				if($this->main->config[$name]->get("chat") === "true"){
					$event->setCancelled();
					foreach($this->main->getServer()->getOnlinePlayers() as $players){
						if($this->main->config[$players->getName()]->get("guild") === $this->main->config[$name]->get("guild")){
							$players->sendMessage("§aGuild>> §f§f".$name." §r: ".$message);
						}
					}
				}
			}
		}elseif(stripos($message, '/kill')){
			$event->setCancelled();
			$player->sendMessage("§c使えません");
		}

		if($this->main->config[$name]->get("start") === 0){
			if($message === "Y"){

			}elseif($message === "N"){
				$this->main->config[$name]->set("start", 11);
			}
		}
	}
}