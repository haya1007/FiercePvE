...
class onQuit{
	...
	public function onQuit($event){
    	...
    	$name = $player->getName();
    	$event->setQuitMessage("§l§c>[§d".$name." Thank for Playing!!§c]<");
		foreach($this->main->config as $data){
			$data->save();
		}
		#$player->save();
		#$this->main->status[$name]->cancel();
		#unset($this->main->status[$name]);
		unset($this->main->slot[$name]);
		if(isset($this->main->tap[strtolower($name)])){
			unset($this->main->tap[strtolower($name)]);
		}
	}
}