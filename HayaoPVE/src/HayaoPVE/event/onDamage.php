...
class onDamage{
	...
	public function onDamage($event){
		...
		if($cause === 7 or $cause === 6 or $cause === 5 or $cause === 13 or $cause === 9 or $cause === 10 or $cause === 1 or $cause === 9 or $cause === 10 or $cause === 8 or $cause === 11 or $cause === 12){
			if(1 > ($entity->getHealth() - $event->getFinalDamage()) or 1 > $entity->getHealth()){
				$this->main->death($entity, "");
				$event->setCancelled();
			}
		}
					...
					if(isset($this->main->eid)){
						foreach($this->main->eid as $eid){
							if(isset($this->main->entity[$eid])){
								$entityplayername = $this->main->entity[$eid]["playername"];
								$entityspeed = $this->main->entity[$eid]["speed"];
								$this->main->showDamage($this->main->entity[$eid]["name"], $eid, $this->main->atk[$name]);
								if($entityplayername === $name and $entityspeed !== 0){
									$this->main->entity[$eid]["target"] = $entity->getid();
								}
							}
						}
					}
