...
class nquest{
	...
	public function onQuest($player){
		////クエスト////
		$npcname = "クエストマン";
		$itemid = 0;
		$pk = new AddPlayerPacket();
		$pk->entityRuntimeId = $this->main->q;
		$pk->uuid = UUID::fromRandom();
		$pk->username = $npcname;
		$pk->position = new Vector3(287.5, 70, 256.5);
	   	$pk->yaw = 0;
	   	$pk->headYaw = 0;
	 	$pk->pitch = 30;
		...
		@$flags |= 0 << Entity::DATA_FLAG_IMMOBILE;
			...
			Entity::DATA_SCALE => [Entity::DATA_TYPE_FLOAT, 0.8],//大きさ
			];
		$geometryJsonEncoded = base64_decode($this->main->c->get("questgeometrydata"));
		if($geometryJsonEncoded !== ""){
			$geometryJsonEncoded = \json_encode(\json_decode($geometryJsonEncoded));
		}
		$skin = new Skin(base64_decode($this->main->c->get("questskinid")), base64_decode($this->main->c->get("questskindata")), base64_decode($this->main->c->get("questcapedata")), base64_decode($this->main->c->get("questgeometryname")), $geometryJsonEncoded);
		...
		$pk2->entityRuntimeId = $this->main->q;
