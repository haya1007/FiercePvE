...
class nsell{
	...
	public function onSell($player){
		...
		$pk->entityRuntimeId = $this->main->b;
		$pk->uuid = UUID::fromRandom();
		$pk->username = $npcname;
		$pk->position = new Vector3(284.5, 70, 292.5);
	   	$pk->yaw = 180;
	   	$pk->headYaw = 190;
	 	$pk->pitch = 0;
		...
		@$flags |= 0 << Entity::DATA_FLAG_IMMOBILE;
			...
			Entity::DATA_SCALE => [Entity::DATA_TYPE_FLOAT, 1.2],//大きさ
			];
		$geometryJsonEncoded = base64_decode($this->main->c->get("sellgeometrydata"));
		if($geometryJsonEncoded !== ""){
			$geometryJsonEncoded = \json_encode(\json_decode($geometryJsonEncoded));
		}
		$skin = new Skin(base64_decode($this->main->c->get("sellskinid")), base64_decode($this->main->c->get("sellskindata")), base64_decode($this->main->c->get("sellcapedata")), base64_decode($this->main->c->get("sellgeometryname")), $geometryJsonEncoded);
		...
		$pk2->entityRuntimeId = $this->main->b;
