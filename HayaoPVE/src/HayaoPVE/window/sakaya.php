...
class sakaya{
	...
	public function onSakaya($id, $data, $p){
		$name = $p->getName();
				if($id === 700){
					if($data == 0){
						$data = [
						...
						$this->main->createWindow($p, $data, 701);
					}
				}elseif($id === 701){
					$m = $this->main->getMoney($p->getName());
					if($data == "true\n"){
						if($m >= 1000){
							$item = Item::get(373, 0, 1)->setCustomName("§l§b水耐性の酒");
							if($p->getInventory()->canAddItem($item)){
								$p->sendMessage("§l§a水耐性の酒を手に入れた!!");
								$p->getInventory()->addItem($item);
								$this->main->removeMoney($p->getName(), 1000);
							}else{
								$p->sendMessage("§l§cインベントリに空きを作ってからまた来てくださいませ");
							}
						}else{
							$p->sendMessage("§l§cお金が足りないです");
						}
					}
				}
	}
