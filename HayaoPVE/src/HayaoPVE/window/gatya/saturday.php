...
class saturday{
	...
	public function onGatya_Saturday($id, $data, $p){
		$name = $p->getName();
				if($id === 951){
					if($data == 0){
						$data = [
						...
						$this->main->createWindow($p, $data, 952);
					}
				}

				if($id === 952){
					if($data == "true\n"){
						$rand = $this->main->gatya_config->get("saturday_count");
						$rand = mt_rand(1, $rand);
						if($rand === 1){
							$this->main->setOrb($name, $this->main->getOrb($name) - 5);
