...
class firedamage extends Task{

	function __construct(PluginBase $owner, $player, $eid, $damage, $bairitu, $kaisuu){
		$this->owner = $owner;
		$this->player = $player;
		$this->eid = $eid;
		$this->damage = $damage;
	...
	function onRun(int $currentTick){
		...
		$eid = $this->eid;
		$damage = $this->damage;
		$bairitu = $this->bairitu;
		if($player instanceof Player){
			#$kaiten = mt_rand(5, $this->kaisuu);
				if(isset($this->owner->entity[$eid]["name"])){
					$name = $player->getName();
					$bai = mt_rand(2, $bairitu);
					$damage = $damage * $bai;
					if($damage < 1){
						$damage = 1;
					}else{
					}
					$this->owner->FloatDamage($player, $eid, $damage, "§c");
					$this->owner->lastattack[$name] = $eid;
					if(isset($this->owner->atkeid[$name][$eid])){
						$this->owner->atkeid[$name][$eid] = $this->owner->atkeid[$name][$eid] + $damage;
					}else{
						$this->owner->atkeid[$name][$eid] = $damage;
					}
					...
					$this->owner->entity[$eid]["hp"] = $this->owner->entity[$eid]["hp"] - $damage;
					$hp = $this->owner->entity[$eid]["hp"];
					if($hp < 1){
						$this->owner->mob_MobDeath->MobDeath($eid,$player);
					}else{
						$rand = mt_rand(1, 25);
						if($rand <= 2){
							$this->owner->getScheduler()->scheduleDelayedTask(new firedamage($this->owner, $player, $eid, $damage, $bairitu, $this->kaisuu), 2);
						}
						...
						$rand = mt_rand(1,5);
						if($this->owner->entity[$eid]["move"] === 0 or $rand === 1){
							$this->owner->entity[$eid]["target"] = $name;
						}
