...
class fly2 extends Task{
	...
	function onRun(int $currentTick){
		$player = $this->player;
		$name = $player->getName();
		if(!$player->isOp() and $player->isFlying()){
				$this->owner->fly[$name] = $this->owner->fly[$name] + 1;
				$player->sendMessage($this->owner->fly[$name]);
		}elseif($this->owner->fly[$name] == 5){
			$player->isBanned();
			$this->owner->broadcastMessage("§c[Fly監視] ".$name."がFlyの疑いで自動的にbanされました");
		}
	}
}