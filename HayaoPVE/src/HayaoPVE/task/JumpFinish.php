...
class JumpFinish extends Task{
	function __construct(PluginBase $owner, $eid, $damage, $level, $plusy, $target, $tnt){
		$this->owner = $owner;
		$this->eid = $eid;
		$this->damage = $damage;
		$this->level = $level;
	...
	function onRun(int $currentTick){
		...
		if(isset($this->owner->entity[$eid])){
			...
			$pk->evid = LevelEventPacket::EVENT_CAULDRON_EXPLODE;
			$pk->position = new Vector3($this->owner->entity[$eid]["x"], $this->owner->entity[$eid]["y"] - $this->plusy, $this->owner->entity[$eid]["z"]);
			$pk->data = 1;
			$pk2 = new LevelSoundEventPacket;
			$pk2->sound = LevelSoundEventPacket::SOUND_EXPLODE;
			$pk2->position = $pk->position;*/

			$pk3 = new MovePlayerPacket();
			$pk3->entityRuntimeId = $eid;
			$pk3->position = new Vector3($this->owner->entity[$eid]["x"], $this->owner->entity[$eid]["y"] - ($this->plusy + 1), $this->owner->entity[$eid]["z"]);
			$pk3->pitch = $this->owner->entity[$eid]["pitch"];
			$pk3->yaw = $this->owner->entity[$eid]["yaw"];
			$pk3->headYaw = $this->owner->entity[$eid]["yaw"];
			foreach($this->level->getPlayers() as $players){
				#$players->dataPacket($pk);
				#$players->dataPacket($pk2);
				$players->dataPacket($pk3);
	 			/*if($players->distanceSquared($pk->position) <= pow($distance,2)){
				...
				$explosion = new Explosion(new Position($this->owner->entity[$eid]["x"], $this->owner->entity[$eid]["y"] - ($this->plusy + 1), $this->owner->entity[$eid]["z"], $this->owner->getServer()->getLevelByName("openworld")), $this->tnt);
				$explosion->explodeB();
			}
			$this->owner->entity[$eid]["move"] = 1;
			$this->owner->entity[$eid]["y"] = $this->owner->entity[$eid]["y"] - ($this->plusy + 1);
			$this->owner->entity[$eid]["target"] = $this->target;
		}
	}
}