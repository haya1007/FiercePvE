...
class PlayerDamage{
	...
	public function PlayerDamage($eid, $target, $damage){
		...
		if(!$protect == 1){
			$name = $this->main->entity[$eid]["name"];
			...
			if($target->getHealth() < $d){
				$this->main->death($target, $eid);
			}else{
				$target->attack($ev);//ダメージ
			}
			$rand = mt_rand(1,10);
			if($target->getX() - $x >= 0){
				$motionx = 1;
