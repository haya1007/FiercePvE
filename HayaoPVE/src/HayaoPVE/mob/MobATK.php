...
class MobATK{
	...
	public function MobATK($eid,$target,$atk){
		$target = $this->main->getServer()->getPlayer($target);
		$level = $target->getLevel();
		$def = $this->main->def[$target->getName()];
		$atkrange = $this->main->entity[$eid]["atkrange"];
		...
		$this->main->mob_MobAttack->MobAttack($mobname, $target, $x, $y, $z, $pos);
		$playerdistance = $this->main->entity[$eid]["playerdistance"];
		if($target->distanceSquared($pos) <= pow($playerdistance,2)){
			$this->main->mob_PlayerDamage->PlayerDamage($eid, $target, $damage);
		}
